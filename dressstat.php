...
<?php
	...
	if(isset($_GET['completeview'])){
		...
		$diff=$diff->format("%a");
		$append="<table><tr><th>Date</th><th>Time</th><th>REG.No</th><th>Name</th><th>Procedure</th></tr>";
		for ($i=0;$i<$diff+1;$i++){
			$query="select d.Date,d.Time,d.Patient_ID,c.Name,d.Procedure_ from dressrec d inner join clientrecord c on d.Patient_ID=c.`REG. NO` AND d.Date=\"".date_format($date1,"Y/m/d")."\"";
	...
	if(isset($_GET['aggregate'])){
		...
		for($i=0;$i<$diff+1;$i++){
			...
			$query="select count(d.Patient_ID),c.Status from dressrec as d inner join clientrecord as c on d.Patient_ID=c.`REG. NO` where Date='$d' group by c.Status";
